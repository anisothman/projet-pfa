        ...
        def wrapper(*args, **kwargs) -> Any:
            ...
            while retries <= max_retries:
                try:
                    ...
                    logger.warning(f"Tentative {retries}/{max_retries} Nouvel essai dans {delay:.1f}s")
                    time.sleep(delay)
...
def validate_response(data: Optional[Dict]) -> Dict:
    if data is None:
    ...
    try:
        ...
        for key in keys:
            ...
            if key not in current:
                logger.debug(f"Clee {key} manquante dans {key_path}")
                return default
            current = current[key]
        if current is None:
